...
class SignUpController extends GetxController {
  ...
  void registerUser(String email, String password) async {
    // String? error = 
    AuthenticationRepository.instance.createUserWithEmailAndPassword(email, password).toString();
    //  if(error != null) {5
  ...
  void createUser(UserModel user) async {
    if ( password.text == ""
    || phoneNo.text == ""|| roles.text == "") {
      Get.showSnackbar(const GetSnackBar(message: "Please Enter The Empty Field",duration: Duration(seconds: 3),));
    } else 
    {
